'use server'

var url = require('url');
import { initializeApp, getApps, getApp } from "firebase-admin/app";

import { getAuth } from "firebase-admin/auth"; // Import getAuth from firebase-admin
import { credential } from "firebase-admin"
// const serviceAccount = require(process.env.GOOGLE_APPLICATION_CREDENTIALS);

const conf = {
  credential: credential.cert({
    "type": "service_account",
    "project_id": "asia-395019",
    "private_key_id": "4664c719c8f3fdea78ced0c890fbcc39c25a0d7d",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "firebase-adminsdk-614iz@asia-395019.iam.gserviceaccount.com",
    "client_id": "117698716301652381202",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-614iz%40asia-395019.iam.gserviceaccount.com",
    "universe_domain": "googleapis.com"
  })
}

const admin = !getApps().length ? initializeApp(conf) : getApp()

export default async function handler(req, res) {
  console.log('...into session')
  const parsedUrl = url.parse(req.url);
  const { query } = parsedUrl; // Assuming the parameter is named 'token'
  const urlParams = new URLSearchParams(query);
  const token = urlParams.get('token');
  try {
    // Verifica il token con Firebase SDK
    const decodedToken = await getAuth().verifyIdToken(token);

    // Ora puoi fare qualcosa con il token decodificato, come impostarlo come cookie
    const firebase_token_cookie = ['firebase-token', `${decodedToken.uid}; HttpOnly; Path=/; SameSite=Strict;`]
    const session_cookie = ['token', `${decodedToken.uid} ; Path=/;`]
    
    res.setHeader('Set-Cookie', [ session_cookie.join('=').concat('Max-Age=31536000'), firebase_token_cookie.join('=')]);
    console.log('redirecting...')
    return res.redirect(307, '/');
  } catch (error) {
    console.error('Errore nella verifica del token:', error);
    return res.json({ success: false, error: 'Token non valido' });
  }

}